...
class Carousel {
  ...
  getCurrentSlideIndex() {
    return [...this.$root.querySelectorAll("ul.carousel-stage > li")].findIndex(($child) => $child.getAttribute("active") === "true");
  }
...
function updateSelectedItemInList(list, newIndex) {
  ...
  currentlySelectedItem == null ? void 0 : currentlySelectedItem.removeAttribute("aria-selected");
...
function updateSelection(block) {
  ...
  const sizes = [...block.querySelectorAll('.sizes-selector > ul > li[aria-selected="true"]')].reduce((total, item) => `${total}${item.innerHTML}`, "");
